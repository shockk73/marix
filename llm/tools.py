...
TOOL_SCHEMAS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "function": {
            "name": "ask_user_form",
            "description": (
                "Задать пользователю НЕСКОЛЬКО вопросов (2–4) с вариантами — "
                "форма. Система задаёт их ПО ОДНОМУ (пользователь отвечает "
                "кнопкой или коротким текстом), а ответы на все вопросы "
                "вернутся одним результатом. Используй вместо цепочки "
                "одиночных ask_user, когда не хватает нескольких параметров "
                "сразу (например дата и окно времени)."
            ),
            "parameters": {
                "type": "object",
                "properties": {
